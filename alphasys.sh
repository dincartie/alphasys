...
ts() {
    local tz_offset sign hours
    tz_offset=$(date +%z)
    sign="${tz_offset:0:1}"
    hours="${tz_offset:1:2}"
    hours="${hours#0}"
    echo "[$(date +%H:%M:%S) UTC${sign}${hours}]"
}
...
warn() { _p "$*"; }
err()  {
    $VERBOSE && echo -e "$(ts) [${_RED}ERROR${_RST}] $*" >&2
    HAD_ERROR=true
}
...
run() {
    ...
    if $VERBOSE; then
        ...
        _tmp=$(mktemp)
        { "$@" 2>&1; echo "$?" > "$_tmp"; } \
            | tee -a /root/alphasys_result.log \
            | while IFS= read -r line; do echo "$(ts)     ${line}"; done
        local _ret
        _ret=$(cat "$_tmp"); rm -f "$_tmp"
        if [ "$_ret" -ne 0 ]; then err "Command failed: $*"; return 1; fi
    else
        if ! "$@" >> /root/alphasys_result.log 2>&1; then
            err "Command failed: $*"
            return 1
        fi
    fi
...
OPTIONS:
  --vmid             Machine identifier (Required when using -mod).
...
EXAMPLE:
  bash alphasys --vmid=102 -mod=hybrid --output
  bash alphasys --vmid=101 -mod=network_setup
  bash alphasys --help
EOF
}

parse_args() {
    local has_help=false has_other=false
    for arg in "$@"; do
        case "$arg" in
            -h|--help)          has_help=true ;;
            --vmid=*)           has_other=true; MACHINE_ID="${arg#--vmid=}" ;;
    ...
    if $has_help; then show_help; exit 0; fi
}

validate_args() {
    if [ -n "$MACHINE_ID" ] && [ -z "$MODULE" ]; then
        echo "Error: The --vmid parameter cannot be used without specifying the module (-mod)." >&2
        exit 1
    fi
    if [ -n "$MODULE" ] && [ -z "$MACHINE_ID" ]; then
        echo "Error: For the module to work, you must specify the machine ID (--vmid)." >&2
        exit 1
    fi
    if [ -n "$MODULE" ]; then
...
ns_hq_rtr() {
    ...
    info "Configuring ens20 trunk (parent for VLANs)"
    ensure_dir /etc/net/ifaces/ens20
    ...
    run firewall-cmd --direct --permanent --add-rule ipv4 filter FORWARD 0 -i ens19 -o tun1 -j ACCEPT
    run firewall-cmd --direct --permanent --add-rule ipv4 filter FORWARD 0 -i tun1 -o ens19 -j ACCEPT
    run systemctl restart firewalld

    info "Installing frr"
    run apt-get install -y frr

    info "Deploying frr config (ospfd=yes)"
    ...
    ok "101 VMID network setup complete."
}
...
ns_hq_srv() {
    ...
    info "Setting resolv.conf: nameserver 77.88.8.8 (pre-apt)"
    ...
    info "Deploying bind zone declarations"
    ...
    info "Restarting bind"
    run systemctl restart bind

    info "Switching resolv.conf: search au-team.irpo / nameserver 127.0.0.1"
    ...
    deploy "network_setup/headquarters/server/etc/openssh/sshd_config" /etc/openssh/sshd_config
    deploy "network_setup/headquarters/server/etc/openssh/banner"      /etc/openssh/banner

    info "Restarting sshd"
    run systemctl restart sshd

    ok "103 VMID network setup complete."
}
...
ns_br_srv() {
    ...
    info "Deploying sshd_config (port 2026, AllowUsers sshuser, MaxAuthTries 2)"
    ensure_dir /etc/openssh
    deploy "network_setup/branch/server/etc/openssh/sshd_config" /etc/openssh/sshd_config
    deploy "network_setup/branch/server/etc/openssh/banner"      /etc/openssh/banner
...
ns_hq_cli() {
    ...
    warn "HQ-CLI uses DHCP. Configure ens19 via the System Control Center GUI:"
    warn "  Main Menu - Show Applications - System Control Center"
    warn "  - Network - Ethernet Interfaces - Configuration: Use DHCP"
...
#  MODULE: network_admin (not yet implemented)
run_network_admin() {
    info "module: network_admin"
    warn "network_admin: no tasks implemented yet."
}

#  MODULE: hybrid (not yet implemented)
run_hybrid() {
    info "module: hybrid"
    warn "hybrid: no tasks implemented yet."
}
...
main() {
    ...
    case "$MODULE" in
        network_setup) run_network_setup ;;
        network_admin) run_network_admin ;;
        hybrid)        run_hybrid        ;;
    esac

    if $HAD_ERROR; then
        _p "  Result: failed.  (see /root/alphasys_result.log for details)"
